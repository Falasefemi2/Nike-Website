    ...
    <>
      ...
      >
        <div className="max-w-lg lg:max-w-none w-full md:text-center grid items-center lg:justify-items-center">
          ...
          <p className="xl:text-sm my-4 text-slate-900">{endpoint.text}</p>
          <a
            href={endpoint.url}
        ...
        <div className="flex items-center justify-center max-w-xl relative lg:max-w-none w-full ">
          <img src={endpoint.img} alt={`img/${endpoint.heading}`} className={`w-auto object-fill transition-theme ${ifExists ? 'h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12' : 'h-72 lg:h-64 md:h-60 sm:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12'}`} />
        </div>
      </div>
    </>
  );
